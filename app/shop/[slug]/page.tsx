...
export default function ShopPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => { params.then(({ slug: nextSlug }) => setSlug(nextSlug)); }, [params]);
  useEffect(() => {
    if (!slug) return;
  ...
  return <main className="min-h-screen bg-white text-neutral-800"><div className="mx-auto max-w-[1240px] px-5 pb-16 pt-5">
    ...
    <div className="grid grid-cols-[220px_1fr] gap-5 pt-4">
      <aside className="text-[13px]"><button type="button" onClick={() => setFiltersOpen(!filtersOpen)} className="flex w-full items-center justify-between border-b border-neutral-100 pb-4 text-left text-[17px] font-semibold">Refine By {filtersOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>{filtersOpen && <div>
      ...
      <section className="min-w-0"><h1 className="py-2 text-center text-[34px] font-normal text-neutral-900">{title}</h1>
